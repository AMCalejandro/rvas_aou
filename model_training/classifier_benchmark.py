...
#                     X_train_scaled = scaler.fit_transform(X_train)
#                     temp_model.fit(X_train_scaled, y_train)
#                     y_train_pred = temp_model.predict_proba(X_train_scaled)[:, 1]
#                 else:
#                     temp_model.fit(X_train, y_train)
#                     y_train_pred = temp_model.predict_proba(X_train)[:, 1]

#                 enforcer = MonotonicityEnforcer()
#                 scores = enforcer.check_monotonicity(X_train, y_train_pred)
#                 features_to_use = enforcer.get_monotonic_features(scores, threshold=0.0)
...
# def evaluate_model_cv(self, X: pd.DataFrame, y: pd.Series, 
    ...
    #         if model_name in ['XGBoost', 'XGBoost (Deep)', 'LightGBM', 'LightGBM (Deep)']:
    #             model_fold.set_params(scale_pos_weight=scale_pos_weight)
    ...
    #             # Train and predict
    #             if sample_weight is not None:
    #                 model_fold.fit(X_train_scaled, y_train, sample_weight=sample_weight)
    #             else:
