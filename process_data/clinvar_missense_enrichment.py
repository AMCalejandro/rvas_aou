...
def _add_build_args(p):
    p.add_argument("--sc-genes-all-path", default=DEFAULT_SC_GENES_ALL_PATH,
                    help="Single-column, header-less gene list used to flag in_scallion_genes.")
    p.add_argument("--output-path", default=DEFAULT_OUTPUT_PATH,
                    help="Where to checkpoint the built ClinVar Hail Table.")
    p.add_argument("--n-partitions", type=int, default=100,
                    help="Number of partitions to coalesce to before checkpointing.")
    p.add_argument("--no-overwrite", action="store_true",
                    help="Fail instead of overwriting an existing table at --output-path.")


def _add_report_args(p):
    p.add_argument("--input-path", default=DEFAULT_OUTPUT_PATH,
                    help="Path to the ClinVar Hail Table built by `build`.")
    p.add_argument("--output-csv", default=None,
                    help="Optional path to also write per-category priors as a CSV.")
...
def _add_select_args(p):
    p.add_argument("--input-path", default=DEFAULT_OUTPUT_PATH,
                    help="Path to the ClinVar Hail Table built by `build`.")
    p.add_argument("--sc-genes-all-path", default=DEFAULT_SC_GENES_ALL_PATH)
    p.add_argument("--sc-genes-multi-path", default=DEFAULT_SC_GENES_MULTI_PATH)
    p.add_argument("--min-missense-variants", type=int, default=MIN_MISSENSE_VARIANTS,
                    help="Floor on classified missense variants/gene before ranking.")
    p.add_argument("--n-genes", type=int, default=N_GENES,
                    help="Number of genes to keep per scallion_label at each extreme.")
    p.add_argument("--z", type=float, default=Z,
                    help="Z-score for the Wilson interval (1.96 ~= 95%%).")
    p.add_argument("--high-output-path", default=DEFAULT_HIGH_ENRICHMENT_OUTPUT,
                    help="Where to export the high-enrichment gene table (TSV).")
    p.add_argument("--low-output-path", default=DEFAULT_LOW_ENRICHMENT_OUTPUT,
                    help="Where to export the low-enrichment gene table (TSV).")
...
def build_arg_parser():
    ...
    build_p = subparsers.add_parser("build", help="Build and checkpoint the filtered ClinVar table.")
    _add_build_args(build_p)

    report_p = subparsers.add_parser("report", help="Report pathogenic priors per functional category.")
    _add_report_args(report_p)

    select_p = subparsers.add_parser("select", help="Select high/low enrichment genes in scallion gene sets.")
    _add_select_args(select_p)

    all_p = subparsers.add_parser("all", help="Run build -> report -> select in sequence.")
    _add_build_args(all_p)
    _add_report_args(all_p)
    # select shares --input-path/--sc-genes-all-path with build/report in "all";
    # only add the select-specific args here to avoid duplicate flags.
    all_p.add_argument("--sc-genes-multi-path", default=DEFAULT_SC_GENES_MULTI_PATH)
    all_p.add_argument("--min-missense-variants", type=int, default=MIN_MISSENSE_VARIANTS)
    all_p.add_argument("--n-genes", type=int, default=N_GENES)
    all_p.add_argument("--z", type=float, default=Z)
    all_p.add_argument("--high-output-path", default=DEFAULT_HIGH_ENRICHMENT_OUTPUT)
    all_p.add_argument("--low-output-path", default=DEFAULT_LOW_ENRICHMENT_OUTPUT)
...
def run_select(args, ht=None):
    if ht is None:
        ht = hl.read_table(args.input_path)
    ...
    high_enrichment.export(args.high_output_path)
    low_enrichment.export(args.low_output_path)
    print(f"Wrote high-enrichment genes to {args.high_output_path}")
    print(f"Wrote low-enrichment genes to {args.low_output_path}")


def main(argv=None):
    hl.init(
        app_name='high and low damaging missense enriched genes ',
        idempotent=True,
        tmp_dir='gs://aou_tmp',
        default_reference="GRCh38",
        gcs_requester_pays_configuration="aou-neale-gwas",
        log="/run_genes_missense_selection.log",
    )
    ...
    if args.command == "build":
        ...
        run_build(args)
        run_report(argparse.Namespace(input_path=args.output_path, output_csv=args.output_csv))
        run_select(argparse.Namespace(
            ...
            n_genes=args.n_genes,
            z=args.z,
            high_output_path=args.high_output_path,
            low_output_path=args.low_output_path,
        ))
