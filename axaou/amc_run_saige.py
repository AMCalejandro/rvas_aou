# DESCRIPTION
# This is the essentially a copy of /Users/am3171/WorkDir/projects/aou_rvas/aou_github/aou_gwas/v8/pipeline/saige_aou_v8.py
# My intention with this script is being able to focus on a smaller part and being able to easily take hands on the outeput
# The goal is to get a better understanding of all the steps around running SAIGE (ie group Fies, bgens,...)

# python3 saige_aou_v8.py --run-pipeline --skip-saige --skip-load-hail-results --skip-any-null-models
# python3 saige_aou_v8.py --run-pipeline --skip-saigƒe --test --skip-load-hail-results --skip-any-null-models --ancestries 'AFR'
...
# SAIGE_DOCKER_IMAGE = "wzhou88/saige:1.5.0.2"
SAIGE_DOCKER_IMAGE = "us-central1-docker.pkg.dev/aou-neale-gwas/saige/saige.multiweights:latest"
# SAIGE_DOCKER_IMAGE = "wzhou88/saige:1.5.1"
HAIL_DOCKER_IMAGE = "hailgenetics/hail:0.2.133-py3.11"
# HAIL_DOCKER_IMAGE = "hailgenetics/hail:0.2.133-py3.9"
# HAIL_DOCKER_IMAGE = "hailgenetics/hail:0.2.137"
...
PILOT_PHENOTYPES = ["random_0.5_continuous_1", "random_0.5_0.01_1", "height"]
# PILOT_PHENOTYPES = ["random_0.5_0.01_1"]

def export_pheno_gate(category: str, pheno_path:str, phenoname: str, fields: str):
    ht = hl.read_table(f'{EXTERNAL_DATA_PATH}/phenotype/{category}/{phenoname}.ht')
    fields = fields.split(',')
...
def export_bgen_from_mt(
    ...
    hl.init(
            ...
            tmp_dir=TMP_BUCKET,
            spark_conf={
                'spark.driver.memory':        '18g',  # ~40g heap, leaving ~12g for OS/overhead/off-heap
                'spark.driver.executor':        '18g',
                'spark.driver.maxResultSize': '2g',
                'spark.memory.fraction':      '0.85', # Give Spark more of the heap for execution
                'spark.memory.storageFraction':'0.2',  # Favor execution over caching (good for BGEN export)
                'spark.local.dir':            '/tmp',
            },
            default_reference="GRCh38",
            log=log_file
    ...
    sample_ids = hl.read_table(f'{EXTERNAL_DATA_PATH}/utils/aou_v8_hard_filter.ht')
    print('Loading MT')
    ...
    call_stats_ht = hl.read_table(f'{EXTERNAL_DATA_PATH}/utils/call_stats/{mt_type.lower() if mt_type == "Exome" else mt_type}_pruned/{ancestry.upper()}_{mt_type.lower() if mt_type == "Exome" else mt_type}_call_stats.ht')
...
def get_filtered_mt(mt_type: hl.tstr,
    ...
    anc_mt_path = f'{EXTERNAL_DATA_PATH}/utils/raw_mt/{mt_type}/{ancestry.upper()}_{mt_type}.mt'
    print(anc_mt_path)
    if ancestry != 'all':
        ancestry_ht = hl.read_table(f'{EXTERNAL_DATA_PATH}/utils/aou_v8_global_pca.ht')
    ...
    if prune_samples:
        ...
        pca_pruned_sample_tsv = f'{EXTERNAL_DATA_PATH}/utils/pca/results/aou_{ancestry.lower()}_centroid_pruned.tsv'
        pca_pruned_sample_path = f'{EXTERNAL_DATA_PATH}/utils/pca/results/{ancestry.lower()}_pca_centroid_pruned.ht'
...
flexrv_hail.py
==============
FlexRV score / MAF weight transformations integrated with Hail Tables.
 
Input schema assumption
-----------------------
The input Hail Tables are already gene-level (keyed by ``gene``), with
variant-level data stored as **parallel arrays** on each row::
 
    Row fields:
        'gene'       : str                    ← key
        'variants'   : array<str>             ← one entry per variant
        'af'         : array<float64>         ← minor allele frequency per variant
        'annotation' : str                    ← e.g. 'missenseother_missense' / 'pLoF'
        'weights'    : array<float64>         ← pathogenicity score per variant
 
Weight strategy
---------------
``annotate_flexrv_weights`` maps over the parallel ``weights`` (score) and
``af`` arrays by index and produces a ``weight_matrix`` field of type
``array<array<float64>>``: one inner array of length 192 per variant.
Position ``i*12 + j`` in the inner array is ``SCORE_WEIGHT_KEYS[i] × MAF_WEIGHT_KEYS[j]``
(row-major, score outer).
 
Pipeline overview
-----------------
1. ``annotate_flexrv_weights()``       – adds ``weight_matrix`` (n_variants × 192)
2. ``merge_missense_lof_gene_tables()``– outer-join the two gene tables
3. ``weight_matrix_to_numpy()`` / ``weights_for_key()`` – NumPy helpers
4. ``cauchy_combination_test()``       – CCT over 192 p-values
5. ``prepare_gene_tables()``           – end-to-end convenience wrapper
"""
...
def _hl_maf_beta_weight(
    ...
    # FlexRV paper (Methods): w = dbeta(MAF/0.001, 1, b) / dbeta(0, 1, b), which
    # expands (Beta(1,b) density = b*(1-x)^(b-1)) to (1 - MAF/0.001)^(b-1) —
    # decreasing in MAF, so rarer variants get more weight. The unweighted
    # `maf / maf_scale` form below (no `1 - `) is `_hl_beta_score_weight`'s own
    # formula, correct for score weighting but backwards for MAF.
    return _hl_beta_score_weight(1.0 - maf / maf_scale, b)
...
def export_gene_group_file(interval, ancestry, mode, target_col, gene_map_source, output_root, n_chunks=1):
    ...
    if mode == 'flexRV':
        ...
        missense_ht = gene_ht.filter(is_missense_annotation_expr(gene_ht))

        missense_ht = missense_ht.annotate(
            gene_mean_score=hl.or_else(gene_mean_ht[gene_label(missense_ht)].gene_mean, hl.float64(0))
        )
        missense_ht = missense_ht.annotate(
            weights=weight_array_expr(missense_ht).map(
                lambda w: hl.or_else(hl.float64(w[target_col]), missense_ht.gene_mean_score)
            )
        )
        ...
        # plof_ht = annotate_unit_weights(plof_grouped, score_field='weights').drop('weights')
        ...
        combined_ht = combined_ht.filter(hl.len(combined_ht.variants) >= MIN_VARIANTS)
        ...
        tmp_ht = (combined_ht
            .annotate(weight_idx=hl.range(N_WEIGHTS))
            .explode('weight_idx'))
        
        weight_key_labels = hl.array(COMBINED_WEIGHT_KEYS)
        weight_ht = (tmp_ht
            .select(
                tag='weight,' + weight_key_labels[tmp_ht.weight_idx],
                info=hl.delimit(
                    tmp_ht.weight_matrix.map(lambda wv: hl.str(_hl_round(wv[tmp_ht.weight_idx]))),
                    ' ',
                ),
            ).key_by())
...
def load_gene_data(directory: str,
    ...
    types = {x: hl.tfloat64 for x in ('MAC', 'Number_rare', 'Number_ultra_rare')}
...
def main(args):
    ...
    if args.export_phenos or (not args.skip_saige) or (not args.skip_any_null_models) or (not args.skip_load_hail_results) or (not args.skip_bgen):
        ...
        if args.pilot or (args.phenos is not None):
            ...
            phenos_to_run_by_ancestry = read_pickle_dict(f'{EXTERNAL_DATA_PATH}/phenotype/summary/pheno_by_ancestry_dict.dict')
            phenos_to_run_by_ancestry_by_group = read_pickle_dict(f'{EXTERNAL_DATA_PATH}/phenotype/summary/pheno_by_ancestry_by_group_dict.dict')
        ...
        for ancestry in ancestries:
            ...
            if args.category is not None:
                ...
                if args.category == 'small':
                    ...
                    phenos_to_run = set(
                        pheno for pheno in phenos_to_run_by_ancestry_by_group[ancestry]["physical_measurement"]
                        if not (pheno.endswith("_male") or pheno.endswith("_female"))
                        )
                ...
                for i in tqdm(range(len(phenos_to_run))):
                    ...
                    if (
                        ...
                        fit_null_task = fit_null_glmm(
                            b,
                            ancestry=ancestry,
                            # output_root=null_glmm_root, # Commented this to avoid overwriting Wenhan's due to mistake running pipeline
                            phenoname=phenoname,
            ...
            if not args.skip_bgen or not args.skip_saige:
                ...
                if args.saige_gene_weight is None:
                    amc_bgen_dir = f"{RESULT_ROOT}/bgen/{args.saige_gene_map_source}/{args.saige_gene_mode}/{ancestry.upper()}"
                else:
                    amc_bgen_dir = f"{RESULT_ROOT}/bgen/{args.saige_gene_map_source}/{args.saige_gene_mode}/{args.saige_gene_weight.upper()}/{ancestry.upper()}"
            ...
            if not args.skip_bgen:
                for interval in intervals:
                    ...
                    # if (f"{amc_bgen_root}.bgen" not in bgens_already_created) or args.overwrite_bgens:
            ...
            if not args.skip_saige:
                ...
                phenos_to_run = list(phenos_to_run)
                print(phenos_to_run)
                print(null_models.keys())
                for i in tqdm(range(len(phenos_to_run))):
                    ...
                    if phenoname not in null_models.keys():
                        logger.warning(f"Skipping {phenoname} — no null model found")
                        continue
                    ...
                    if ( not overwrite_results and hfs.exists(pheno_results_dir)):
                        results_already_created = {x["path"] for x in hl.hadoop_ls(pheno_results_dir)}
                    if ( not args.overwrite_exome_results and hfs.exists(pheno_results_dir) and analysis_type == 'gene'):
                        exome_results_already_created = {x["path"] for x in hl.hadoop_ls(pheno_results_dir) if
                                                         x["path"].endswith('_exome.single_variant.txt')}
                        print(f'-------------- Found {int(len(exome_results_already_created))} exome results in [{ancestry}: {phenoname}] directory...')
                    ...
                    for interval in intervals:
                        ...
                        #         memory = '10G' if ancestry in ['eur', 'all']  else 'standard'
            ...
            if not args.skip_load_hail_results:
                ...
                # root = f'{RESULTS_PATH}/ht_results/{}/{}{ancestry.upper()}'
                if args.saige_gene_weight is None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(""
    "--run-pipeline", help="Run SAIGE pipeline", action="store_true"
    )
    parser.add_argument(
